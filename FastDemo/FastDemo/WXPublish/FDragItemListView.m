...
@implementation FDragItemListView
...
- (void)addItem:(FDragItem *)item
{
    ...
    if (self.items.count && self.items.count == _maxItem) {
        FDragItem *last = [self.items lastObject];
    ...
    FDragItem *addItem = nil;
    if (self.items.count) {
        FDragItem *lastItem = [self.items lastObject];
        if (lastItem.isAdd) {
            addItem = lastItem;
            tag = lastItem.tag;
            lastItem.tag = tag +1;
...
- (void)clickItem:(UITapGestureRecognizer *)tap
{
    [self dismissKeyBord];
    if (_clickItemBlock) {
        _clickItemBlock((FDragItem *)tap.view);
    }
}
...
- (void)pan:(UIPanGestureRecognizer *)pan
{
    ...
    FDragItem *tagButton = (FDragItem *)pan.view;
    ...
    if (pan.state == UIGestureRecognizerStateChanged) {
        ...
        // 获取当前按钮中心点在哪个按钮上
        FDragItem *otherButton = [self buttonCenterInButtons:tagButton];
...
// 看下当前按钮中心点在哪个按钮上
- (FDragItem *)buttonCenterInButtons:(FDragItem *)curItem
{
    for (FDragItem *button in self.items) {
...
- (void)updateItemFrame:(NSInteger)i extreMargin:(BOOL)extreMargin
{
    ...
    // 获取当前按钮
    FDragItem *tagItem = self.items[i];
...
// 计算标签按钮frame（按规律排布）
- (void)setupItemButtonRegularFrame:(FDragItem *)tagItem
{
    // 获取角标
